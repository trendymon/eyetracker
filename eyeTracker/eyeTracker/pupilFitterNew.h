...
*/
	bool pupilAreaFitRR(Mat& gray, RotatedRect &rr, vector<Point> &allPtsReturn,
			...
			int sizeIn = 240, int darkestPixelL1In = 10, int darkestPixelL2In = 20)
	{
		...
		//find pupil
		Point darkestPixelConfirm = getDarkestPixelArea(gray(cv::Rect(pupilSearchXMin, pupilSearchYMin, size, size)));
		...
		cv::findContours(threshLow, contoursLow, RETR_EXTERNAL, CHAIN_APPROX_TC89_L1);
		...
		cv::findContours(threshHigh, contoursHigh, RETR_EXTERNAL, CHAIN_APPROX_TC89_L1);
		...
		Canny(threshLow, thresh3, lowThresholdCanny, lowThresholdCanny*ratio, kernel2);
		Canny(threshHigh, thresh4, highThresholdCanny, highThresholdCanny*ratio, kernel2);
		if (threshDebug) {
		...
		RotatedRect ellipseCorrect = RotatedRect(Point(0, 0), Size2f(0, 0), 0);
		...
		if (allPts.size() > 5) {
			...
			RotatedRect ellipseRaw = fitEllipse(allPts);
			ellipseCorrect = RotatedRect(Point(ellipseRaw.center.x + darkestPixelConfirm.x, ellipseRaw.center.y + darkestPixelConfirm.y), ellipseRaw.size, ellipseRaw.angle);
		...
		///waitKey(1);
		for (int i = 0; i < allPts.size(); i++) {
			allPtsReturn.push_back(Point(darkestPixelConfirm.x, darkestPixelConfirm.y));
		}
		rr = ellipseCorrect;		
		return true;
	}
		
	bool pupilAreaFitRRNew(Mat& gray, RotatedRect &rr, vector<Point> &allPtsReturn,
		int pupilSearchAreaIn = 10, int pupilSearchXMinIn = 0, int pupilSearchYMinIn = 0,
		int lowThresholdCannyIn = 10, int highThresholdCannyIn = 30,
		int sizeIn = 240, int darkestPixelL1In = 10, int darkestPixelL2In = 20)
	{
		...
		cv::Mat ROI(gray(cv::Range(pupilSearchYMinIn, 480 - pupilSearchYMinIn), cv::Range(pupilSearchXMinIn, 640 - pupilSearchXMinIn)));

		threshold(ROI, threshLow, darkestPixelL1In, 255, 1);		
		cv::GaussianBlur(ROI, ROI, cv::Size(19, 19), 0);
		...
		Mat element = getStructuringElement(erosion_type,
			Size(2 * erosion_size + 1, 2 * erosion_size + 1),
			Point(erosion_size, erosion_size));
		cv::dilate(threshLow, threshLow, element);

		cv::Canny(threshLow, thresh3, lowThresholdCanny, lowThresholdCanny*3, 3);
		//cv::HoughCircles(thresh3, thresh3, CV_HOUGH_GRADIENT, 0, 10);
...
*/
	int getDarkestPixel(const Mat& I)
	{
...
// new Version
	Point getDarkestPixelArea(const Mat& I)
	{		
		cv::GaussianBlur(I, I, cv::Size(9, 9), 0);
		Point minLoc;		
		cv::minMaxLoc(I, nullptr, nullptr, &minLoc);
		minLoc = cv::Point(minLoc.x + pupilSearchXMin, minLoc.y + pupilSearchYMin);		
		return minLoc;
	}
...
*/
	vector<Point> getCandidates(std::vector<std::vector<cv::Point>>contours, int biggest, Mat& thresh, bool draw){
		...
		//debug
		const bool candidatesOn = draw;
		const bool cannyOn = draw;
		...
		if (contours[biggest].size() > 10){
			for (int i = 0; i < contours[biggest].size(); i++){
				const int x = contours[biggest][i].x;
				const int y = contours[biggest][i].y;
				int mult = 2;
...
//Better fits a set of candidate points to a pupil ellipse
	vector<Point> refinePoints(vector<Point> allPts, Mat gray, int checkThickness, int checkSpacing, Mat grayOriginal, bool rmOutliers = false){
		...
		for (int i = 0; i < allPts.size(); i++){
			...
			int finalX = allPts[i].x;
			int finalY = allPts[i].y;
			...
			if (allPts[i].x - checkThickness*checkSpacing - 1 >= 0 &&
				...
				for (int j = -checkThickness*checkSpacing; j < checkThickness*checkSpacing; j = j + checkSpacing){
					...
					int centerValue = gray.at<uchar>(Point(allPts[i].x + j, allPts[i].y));
					...
					int leftDiff = abs(centerValue - gray.at<uchar>(Point(allPts[i].x + j - 1, allPts[i].y)));
					int rightDiff = abs(centerValue - gray.at<uchar>(Point(allPts[i].x + j + 1, allPts[i].y)));
			...
			if (allPts[i].y - checkThickness*checkSpacing - 1 >= 0 &&
				...
				for (int j = -checkThickness*checkSpacing; j < checkThickness*checkSpacing; j = j + checkSpacing){
					...
					int centerValue2 = gray.at<uchar>(Point(allPts[i].x, allPts[i].y + j));

					//calculate and find diffs
					int topDiff = abs(centerValue2 - gray.at<uchar>(Point(allPts[i].x, allPts[i].y + j - 1)));
					int botDiff = abs(centerValue2 - gray.at<uchar>(Point(allPts[i].x, allPts[i].y + j + 1)));
